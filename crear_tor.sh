...
echo "[ruta];An+;AnON;Tipo recurso;Titulo;Descripción;POSX;POSY;URL o path y nombre del archivo;Perfil;Nivel" >> $salida
...
for f in $( find -mindepth 2 -name "*.zip" | sort ); do

	echo ""
	nombre_orig=$(basename $f)
	nombre=$(basename $f)
	carpeta=$(dirname $f)
	...
	pagina=$(echo $nombre_orig | egrep -o 'p[0-9]+')
	pagina="${pagina//p/}"
	pagina=$(echo $pagina | sed 's/^0*//')
	campoan="$pagina"

	unidad=$(echo $nombre_orig | egrep -o 'U[0-9]+' )
	unidad="${unidad//U/}"
	unidad=$(echo $unidad | sed 's/^0*//')
	((unidad++))
	campoanon="T""$unidad"

	echo $campotitulo
	echo "$camporuta;$campoan;$campoanon;indice;$campotitulo;;;;$nombre;;4" >> $salida
